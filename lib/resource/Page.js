...
module.exports = function (Resource) {
    ...
    if (useMinJs) {
        extList.push('js');
        whiteList.push(whiteListJs);
    }
    if (useMinCss) {
        extList.push('css');
        whiteList.push(whiteListCss);
    }
        ...
        render: function (res, options, callback) {
            ...
            res.render(this.pageRenderID, {data: opts}, function (err, html) {
                ...
                if (html) {
                    ...
                    // 替换.js文件为min.js
                    if (extList.length > 0) {
                        html = scriptLoader.replaceWithMin(html, extList, whiteList);
                    }
                    /*
                    if (useMinJs) {
                        html = scriptLoader.replaceWithMinOld(html, 'js', whiteListJs);
                    }
                    if (useMinCss) {
                        html = scriptLoader.replaceWithMinOld(html, 'css', whiteListCss);
                    }
                    */
                }
                if (done) {
