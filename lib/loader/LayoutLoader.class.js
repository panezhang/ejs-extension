...
_.extend(LayoutLoader.prototype, {
    ...
    render: function (html) {
        ...
        _.each(blockList, function (blockName) {
            ...
            var outerStartPos = html.indexOf(startTag);
            var startPos = outerStartPos + startTag.length;
            var endPos = html.indexOf(endTag);
            var outerEndPos = endPos + endTag.length;

            var innerHtml = html.substring(startPos, endPos);

            self.logger.trace(startPos, endPos);
            if (startPos >= startTag.length && endPos > startTag.length) {
                // remove outer content
                html = html.substring(0, outerStartPos) + html.substr(outerEndPos);
                // replace tag with innerHtml
                html = html.replace(defineTag, innerHtml);

                // 正则表达式效率过低，页面太长时导致堆栈溢出
