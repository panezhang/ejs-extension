...
module.exports = function (opts) {
    ...
    return {
        ...
        write: function (view, options, uaIdentity, str) {
            ...
            logger.error('write cache success, current caches for', view, _.pluck(pool, 'hit'));
        },
        ...
        read: function (view, options, uaIdentity) {
            ...
            if (found && found.expireStamp > new Date().getTime()) {
                found.hit = found.hit + 1;
                logger.error('total cache copy: %s, found cache for: ', pool.length, view, _.pluck(pool, 'hit'));
                return found.str;
            } else {
